...
export function LogitLensWidget(
  ...
  const uid = "ll_interact_" + instanceCount++;
  ...
  // Event handlers
  let eventHandlers: WidgetEventHandlers = {};
  ...
  function getSerializedPinnedRows(): SerializedPinnedRow[] {
    return state.pinnedRows.map((pr) => ({
      pos: pr.pos,
      lineStyleName: pr.lineStyle.name,
    }));
  }

  // Fire row pin change event
  function fireRowPinChange(): void {
    if (eventHandlers.onRowPinChange) {
      eventHandlers.onRowPinChange(getSerializedPinnedRows());
    }
  }

  // Fire group pin change event
  function fireGroupPinChange(): void {
    if (eventHandlers.onGroupPinChange) {
  ...
  if (uiState?.pinnedRows) {
    state.pinnedRows = uiState.pinnedRows.map((pr) => {
      const lineStyle =
        LINE_STYLES.find((ls) => ls.name === pr.lineStyleName) || LINE_STYLES[0];
  ...
  function getTrajectoryForToken(token: string, pos: number): number[] {
  ...
  // Get rank trajectory from original V2 data
  function getRankTrajectoryForToken(token: string, pos: number): number[] {
    const v2Data = widgetData as V2InputData;
  ...
  function getGroupTrajectory(group: PinnedGroup, pos: number): number[] {
    if (trajectoryMetric === "rank") {
      ...
      // Replace Infinity with 0 for missing data
